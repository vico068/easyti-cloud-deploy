<div>
    <x-slot:title>
        {{ data_get_str($application, 'name')->limit(10) }} > Configuração | EasyTI Cloud
    </x-slot>
    
    {{-- EASYTI: Título traduzido --}}
    <h1>{{ isEasytiAdmin() ? 'Configuração' : '' }}</h1>
    ...
    <div class="flex flex-col h-full gap-8 sm:flex-row">
        ...
        <div class="flex flex-col items-start gap-2 min-w-fit">
            ...
            <a class='menu-item' {{ wireNavigate() }} wire:current.exact="menu-item-active"
                href="{{ route('project.application.advanced', ['project_uuid' => $project->uuid, 'environment_uuid' => $environment->uuid, 'application_uuid' => $application->uuid]) }}">Avançado</a>
            @if ($application->destination->server->isSwarm())
                <a class="menu-item" {{ wireNavigate() }} wire:current.exact="menu-item-active"
                    href="{{ route('project.application.swarm', ['project_uuid' => $project->uuid, 'environment_uuid' => $environment->uuid, 'application_uuid' => $application->uuid]) }}">Configuração Swarm</a>
            @endif
            <a class='menu-item' {{ wireNavigate() }} wire:current.exact="menu-item-active"
                href="{{ route('project.application.environment-variables', ['project_uuid' => $project->uuid, 'environment_uuid' => $environment->uuid, 'application_uuid' => $application->uuid]) }}">Variáveis de Ambiente</a>
            <a class='menu-item' {{ wireNavigate() }} wire:current.exact="menu-item-active"
            ...
            <a class="menu-item flex items-center gap-2" {{ wireNavigate() }} wire:current.exact="menu-item-active"
                ...
                @if ($application->server_status == false)
                    <span title="Um ou mais servidores estão inacessíveis.">
                ...
                @elseif ($application->additional_servers()->exists() && str($application->status)->contains('degraded'))
                    <span title="Aplicação em estado degradado.">
                        <svg class="w-4 h-4 text-error" viewBox="0 0 256 256" xmlns="http://www.w3.org/2000/svg">
            ...
            <a class="menu-item" {{ wireNavigate() }} wire:current.exact="menu-item-active"
                href="{{ route('project.application.resource-operations', ['project_uuid' => $project->uuid, 'environment_uuid' => $environment->uuid, 'application_uuid' => $application->uuid]) }}">Operações</a>
            <a class="menu-item" {{ wireNavigate() }} wire:current.exact="menu-item-active"
                href="{{ route('project.application.metrics', ['project_uuid' => $project->uuid, 'environment_uuid' => $environment->uuid, 'application_uuid' => $application->uuid]) }}">Métricas</a>
            <a class="menu-item" {{ wireNavigate() }} wire:current.exact="menu-item-active"
