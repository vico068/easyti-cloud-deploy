...
class Team extends Model implements SendsDiscord, SendsEmail, SendsPushover, SendsSlack
{
    ...
    protected $casts = [
        ...
        'is_master' => 'boolean',  // EASYTI: Campo para team master
        'plan_limits' => 'array',   // EASYTI: Limites por plano
    ];

    // EASYTI: Limites padrão por plano
    ...
    // EASYTI: Verificar se é o team master (Easy TI Solutions - admin)
    public function isSuperAdmin(): bool
    {
        // Verifica se é o primeiro team (ID 0 ou 1) OU se tem flag is_master
        // Isso garante que funcione mesmo antes da migração rodar
        return $this->id === 0 || $this->id === 1 || ($this->is_master ?? false) === true;
    }

    // EASYTI: Obter o servidor do team master para clientes
    public function getMasterServer()
    {
        // Se for o team master, retorna o próprio servidor
        if ($this->isSuperAdmin()) {
            return $this->servers()->first();
        }

        // Caso contrário, usa o servidor do team master
        $masterTeam = Team::where('is_master', true)->first();
        return $masterTeam?->servers()->first();
    }

    // EASYTI: Obter limites do plano atual
    public function getPlanLimits(): array
    {
        // Se tiver limites customizados, usa eles
        if ($this->plan_limits) {
            return array_merge(self::PLAN_LIMITS[$this->plan] ?? self::PLAN_LIMITS['starter'], $this->plan_limits);
        }

        return self::PLAN_LIMITS[$this->plan] ?? self::PLAN_LIMITS['starter'];
    }

    // EASYTI: Verificar se atingiu limite de aplicações
    public function hasReachedApplicationLimit(): bool
    {
        $limits = $this->getPlanLimits();
        $maxApps = $limits['max_applications'] ?? 3;
        ...
        return $this->applications()->count() >= $maxApps;
    }

    // EASYTI: Obter Docker network do team
    public function getDockerNetworkName(): string
    {
        return "easyti_team_{$this->id}_network";
    }
}
