...
class Application extends BaseModel
{
    ...
    // EASYTI: Container name com prefixo do team para isolamento
    public function getEasytiContainerName(): string
    {
        $team = $this->team();
        $teamId = $team ? $team->id : 0;
        return "team_{$teamId}_{$this->uuid}";
    }

    // EASYTI: Docker network isolada por team
    public function getEasytiDockerNetwork(): string
    {
        $team = $this->team();
        $teamId = $team ? $team->id : 0;
        return "easyti_team_{$teamId}_network";
    }

    // EASYTI: Verificar se o team pode criar mais aplicações
    public function canTeamCreateApplication(): bool
    {
        $team = $this->team();
        if (! $team) {
            return false;
        ...
        // Admins podem sempre criar
        if ($team->isSuperAdmin()) {
            return true;
        }

        return ! $team->hasReachedApplicationLimit();
    }
